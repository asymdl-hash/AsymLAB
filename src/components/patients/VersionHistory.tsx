...
export default function VersionHistory({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="bg-white border border-gray-200 rounded-xl w-full max-w-lg max-h-[80vh] overflow-hidden shadow-2xl">
                {/* Header */}
                <div className="flex items-center justify-between p-4 border-b border-gray-200">
                    ...
                    <button onClick={onClose} className="text-gray-500 hover:text-gray-700 text-xs">
                        Fechar ✕
                    </button>
                </div>
...
function VersionCard({ label, fields, conteudo, editorName, editedAt, isExpanded, onToggle, isCurrent }: {
    ...
    return (
        <div className={`border rounded-lg overflow-hidden ${isCurrent ? 'border-amber-500/30 bg-amber-500/5' : 'border-gray-200 bg-gray-50'}`}>
            <button
                onClick={onToggle}
            ...
            {isExpanded && (
                <div className="px-3 pb-3 border-t border-gray-200/50 space-y-2 mt-0">
                    {fields.length > 0 && fields.map((f, i) => (
                        <div key={i} className="mt-2">
                            <span className="text-[10px] uppercase tracking-wider text-gray-500">{f.subtitulo}</span>
                            <p className="text-sm text-gray-700 mt-0.5">{f.descricao || <span className="italic text-gray-400">Vazio</span>}</p>
                        </div>
                    ))}
                    {conteudo && (
                        <div className="mt-2">
                            <span className="text-[10px] uppercase tracking-wider text-gray-500">Texto</span>
                            <p className="text-sm text-gray-700 mt-0.5 whitespace-pre-wrap">{conteudo}</p>
                        </div>
                    )}
                </div>
            )}
